...
test.describe("Report Claiming and Clearing", () => {
  test.describe("Claim Report", () => {
    test("should claim a pending report", async ({ page }) => {
      ...
      const pendingReport = page
        .locator('.report-card:has-text("Pending")')
        .first();
      ...
      await expect(
        page.locator("text=/claimed.*successfully|successfully.*claimed/i"),
      ).toBeVisible({
        timeout: 5000,
      });
      ...
      await expect(
        page.locator("text=/status.*claimed|claimed/i"),
      ).toBeVisible();
    });
    ...
    test("should not allow claiming already claimed report", async ({
      page,
    }) => {
      const user = generateRandomUser();
      ...
      const claimedReport = page
        .locator('.report-card:has-text("Claimed")')
        .first();
    ...
    test("should update report list after claiming", async ({ page }) => {
      ...
      if (reportId) {
        const reportInList = page.locator(
  ...
  test.describe("Clear Report", () => {
    test("should clear a claimed report with after photo", async ({ page }) => {
      ...
      // Should show success message
      await expect(
        page.locator("text=/cleared.*successfully|marked.*cleared/i"),
      ).toBeVisible({
        timeout: 15000,
      });
      ...
      await expect(
        page.locator("text=/status.*cleared|cleared/i"),
      ).toBeVisible();
    });
    ...
    test("should not allow clearing without after photo", async ({ page }) => {
      ...
      // Should show error
      await expect(
        page.locator("text=/photo.*required|upload.*photo/i"),
      ).toBeVisible();
    });

    test("should only allow clearer to mark report as cleared", async ({
      page,
      context,
    }) => {
    ...
    test("should show both before and after photos after clearing", async ({
      page,
    }) => {
      const user = generateRandomUser();
      ...
      await expect(
        page.locator('img[alt*="before"], img[alt*="Before"]'),
      ).toBeVisible();
      await expect(
        page.locator('img[alt*="after"], img[alt*="After"]'),
      ).toBeVisible();
    });
    ...
    test("should award points after clearing report", async ({ page }) => {
      ...
      const initialPointsText = await page
        .locator('[data-testid="user-points"], text=/points/i')
        .textContent();
  ...
  test.describe("My Cleared Reports", () => {
    test("should display reports cleared by user", async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="my-clears-list"]'),
      ).toBeVisible();
    });

    test("should show empty state when user has not cleared any reports", async ({
      page,
    }) => {
      const user = generateRandomUser();
      ...
      await expect(
        page.locator("text=/no.*cleared|haven't cleared/i"),
      ).toBeVisible();
    });

    test("should show cleared reports with before and after photos", async ({
      page,
    }) => {
      const user = generateRandomUser();
  ...
  test.describe("Claim and Clear Workflow", () => {
    test("should complete full claim and clear workflow", async ({ page }) => {
      ...
      await expect(
        page.locator("text=/cleared.*successfully|status.*cleared/i"),
      ).toBeVisible({
        timeout: 15000,
      });
