...
test.describe('Verification Workflow', () => {
	test.describe('Verification Queue', () => {
		test('should display cleared reports in verification queue', async ({ page }) => {
			...
			await expect(page.locator('.report-card:has-text("Cleared")')).toBeVisible();
		});

		test('should not show reports that user has already verified', async ({ page }) => {
			const user = generateRandomUser();
		...
		test('should not show reports that user cleared themselves', async ({ page }) => {
			const user = generateRandomUser();
		...
		test('should show reporter can verify someone else\'s cleanup of their report', async ({ page }) => {
		...
		test('should show minimum clears requirement', async ({ page }) => {
			...
			await expect(page.locator('text=/need.*clear|clear.*\d+.*reports/i')).toBeVisible();
		});
	...
	test.describe('Submit Verification', () => {
		test('should verify a cleared report positively', async ({ page }) => {
			...
			await page.click('button[value="true"], button:has-text("Yes"), button:has-text("Verified")');
			...
			await expect(page.locator('text=/verification.*submitted|verified.*successfully/i')).toBeVisible({
				timeout: 5000,
			});
		});
		...
		test('should verify a cleared report negatively', async ({ page }) => {
			...
			await page.click('button[value="false"], button:has-text("No"), button:has-text("Not Verified")');
			...
			// Should show success message
			await expect(page.locator('text=/verification.*submitted/i')).toBeVisible();
		});

		test('should require comment for negative verification', async ({ page }) => {
			const user = generateRandomUser();
		...
		test('should award points for verification', async ({ page }) => {
			...
			const initialPointsText = await page.locator('[data-testid="user-points"]').textContent();
			...
			await page.goto('/app/profile');
			const newPointsText = await page.locator('[data-testid="user-points"]').textContent();
	...
	test.describe('View Verifications', () => {
		test('should display verifications for a report', async ({ page }) => {
			...
			const clearedReport = page.locator('.report-card:has-text("Cleared")').first();
			await clearedReport.click();

			// Should show verifications section
			await expect(page.locator('text=/verifications|verified by/i')).toBeVisible();
		});
		...
		test('should display positive and negative verifications separately', async ({ page }) => {
			const user = generateRandomUser();
		...
		test('should show verification comments', async ({ page }) => {
			...
			const verificationsSection = page.locator('[data-testid="verifications-list"]');
			
			if (await verificationsSection.isVisible()) {
				// Should show at least one comment
				await expect(verificationsSection.locator('text=/comment/i')).toBeVisible();
			}
		});
	});

	test.describe('Report Status Progression', () => {
		test('should mark report as verified after minimum positive verifications', async ({ page, context }) => {
		...
		test('should award bonus points to clearer when report is verified', async ({ page }) => {
		...
		test('should show verified badge on verified reports', async ({ page }) => {
			...
			const verifiedReport = page.locator('.report-card:has-text("Verified")').first();
			
			if (await verifiedReport.isVisible()) {
				// Should have a verified badge or indicator
				await expect(verifiedReport.locator('[data-testid="verified-badge"], .verified-badge')).toBeVisible();
			}
		});
	});

	test.describe('Verification Experience Requirements', () => {
		test('should prevent verification before minimum clears', async ({ page }) => {
			...
			await expect(page.locator('text=/need to clear.*\d+.*reports/i')).toBeVisible();
		...
		test('should unlock verification after completing minimum clears', async ({ page }) => {
			const user = generateRandomUser();
