...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_reports_cleared_at 
    ON litter_reports(cleared_at DESC) 
    WHERE cleared_by IS NOT NULL;

-- Composite index for geospatial + time queries (scoring service)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_reports_cleared_location_time
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_verifications_report_verified
    ON report_verifications(report_id, is_verified);

-- Index for user score ordering (leaderboards)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_user_scores_total_points
    ON user_scores(total_points DESC);

-- Index for cleared reports count (leaderboards)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_user_scores_reports_cleared
    ON user_scores(reports_cleared DESC);

-- Index for finding user's reports
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_reports_reporter_created
    ON litter_reports(reporter_id, created_at DESC);

-- Index for finding user's claimed reports
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_reports_claimed_by
    ON litter_reports(claimed_by, claimed_at DESC)
    WHERE claimed_by IS NOT NULL;

-- Index for finding user's cleared reports  
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_reports_cleared_by
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_email_tokens_user_expires
