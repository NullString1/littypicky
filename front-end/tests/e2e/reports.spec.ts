...
test.describe("Report Management", () => {
  test.describe("Create Report", () => {
    test("should create a new litter report with photo", async ({ page }) => {
      ...
      await expect(
        page.locator("text=/report.*submitted|success/i"),
      ).toBeVisible({
        timeout: 15000,
      });
    ...
    test("should show error when submitting without photo", async ({
      page,
    }) => {
      const user = generateRandomUser();
      const reportData = generateReportData();
    ...
    test("should prevent creating report without verified email", async ({
      page,
    }) => {
      const user = generateRandomUser();
  ...
  test.describe("View Reports", () => {
    test("should display nearby reports on feed page", async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="reports-list"], .report-card'),
      ).toBeVisible({
        timeout: 10000,
      });
      ...
      await expect(
        page.locator("text=/pending|claimed|cleared/i"),
      ).toBeVisible();
    });
    ...
    test("should filter reports by status", async ({ page }) => {
      ...
      await expect(
        page.locator('.report-card:has-text("Claimed")'),
      ).not.toBeVisible();
      await expect(
    ...
    test("should show report details when clicked", async ({ page }) => {
      ...
      await expect(page).toHaveURL(/\/app\/report\/[a-f0-9-]+/, {
        timeout: 5000,
      });

      // Should show report details
      await expect(
        page.locator("text=/description|location|status/i"),
      ).toBeVisible();
    });
    ...
    test("should display report photo", async ({ page }) => {
      ...
      const reportPhoto = page
        .locator('.report-card img, [alt*="report"]')
        .first();
    ...
    test("should update feed when new reports are created", async ({
      page,
      context,
    }) => {
      const user = generateRandomUser();
  ...
  test.describe("View My Reports", () => {
    test("should display user's own reports", async ({ page }) => {
      ...
      // Should show reports list
      await expect(
        page.locator('[data-testid="my-reports-list"]'),
      ).toBeVisible();
    });

    test("should show empty state when user has no reports", async ({
      page,
    }) => {
      const user = generateRandomUser();
      ...
      await expect(
        page.locator("text=/no reports|haven't created/i"),
      ).toBeVisible();
    });
  });
  ...
  test.describe("Report Details", () => {
    test("should display full report information", async ({ page }) => {
      ...
      await expect(
        page.locator("text=/location|coordinates|latitude|longitude/i"),
      ).toBeVisible();
      await expect(
        page.locator('img[alt*="before"], img[alt*="litter"]'),
      ).toBeVisible();
    });

    test("should show before and after photos for cleared reports", async ({
      page,
    }) => {
      const user = generateRandomUser();
